...
export default function UsersPage() {
    ...
    const loadData = async () => {
        setIsLoading(true);
        try {
            const result = await getUsersWithAnalytics(token);
            setData(result);
        } catch (error) {
            console.error(error);
        } finally {
    ...
    const handleSubscriptionSubmit = async (e: React.FormEvent) => {
        ...
        const formData = new FormData(e.target as HTMLFormElement);
        try {
            await updateSubscription(token, selectedSubscription.id, {
    ...
    const RoleBadge = ({ role }: { role: string }) => {
        return (
            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${role === "ADMIN"
