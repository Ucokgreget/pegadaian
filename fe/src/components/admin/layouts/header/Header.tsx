      ...
      <div className="absolute top-[calc(100%+10px)] right-0 min-w-[300px] max-sm:min-w-0 max-sm:w-[calc(100vw-2rem)] max-sm:-right-2 rounded-2xl border border-border bg-card shadow-[0_4px_6px_rgba(0,0,0,0.08),0_16px_40px_rgba(0,0,0,0.18),0_0_0_1px_rgba(34,197,94,0.08)] overflow-hidden z-[100] animate-in slide-in-from-top-2 fade-in duration-200 origin-top-right before:absolute before:-top-[5px] before:right-3.5 before:w-2.5 before:h-2.5 before:bg-card before:border-l before:border-t before:border-border before:rotate-45 before:rounded-sm">
        <div className="flex items-center justify-between pt-3.5 px-4 pb-2.5 border-b border-border">
          <span className="text-sm font-bold text-foreground flex items-center gap-1.5">
            Notifikasi
            {unreadCount > 0 && (
              <span className="inline-flex items-center justify-center w-[18px] h-[18px] rounded-full bg-green-500 text-white text-[0.65rem] font-bold">{unreadCount}</span>
            )}
          </span>
          {unreadCount > 0 && (
            <button className="text-[0.72rem] font-semibold text-green-500 bg-transparent border-none cursor-pointer p-0 font-jakarta hover:text-green-600" onClick={onMarkAll}>
              Tandai semua dibaca
            </button>
          )}
        </div>
            ...
            notifications.map((n) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[0.8rem] font-medium text-foreground leading-snug">{n.text}</div>
                  <div className="text-[0.7rem] text-muted-foreground mt-0.5">{n.time}</div>
                </div>
              </div>
            ))
          )}
        ...
        <div className="p-2 border-t border-border">
          <button className="w-full p-2 rounded-lg bg-transparent font-jakarta text-[0.8rem] font-semibold text-green-500 cursor-pointer transition-colors duration-150 hover:bg-green-500/10" onClick={onClose}>
    ...
    <>
      ...
      <div className="absolute top-[calc(100%+10px)] right-0 min-w-[240px] max-sm:min-w-0 max-sm:w-[220px] rounded-2xl border border-border bg-card shadow-[0_4px_6px_rgba(0,0,0,0.08),0_16px_40px_rgba(0,0,0,0.18),0_0_0_1px_rgba(34,197,94,0.08)] overflow-hidden z-[100] animate-in slide-in-from-top-2 fade-in duration-200 origin-top-right before:absolute before:-top-[5px] before:right-3.5 before:w-2.5 before:h-2.5 before:bg-card before:border-l before:border-t before:border-border before:rotate-45 before:rounded-sm">
        <div className="pt-4 px-4 pb-3 flex items-center gap-3 border-b border-border">
          <div className="w-11 h-11 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center text-[1.1rem] font-bold text-white shrink-0">{getInitials(user?.name)}</div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-bold text-foreground whitespace-nowrap overflow-hidden text-ellipsis">{user?.name || "User"}</div>
            <div className="text-[0.7rem] text-muted-foreground whitespace-nowrap overflow-hidden text-ellipsis mt-[1px]">{user?.email || "-"}</div>
            <div className="inline-flex items-center rounded-full bg-green-500/10 border border-green-500/20 px-2 py-0.5 text-[0.62rem] font-bold text-green-500 uppercase tracking-[0.06em] mt-1">
              {user?.role === "ADMIN" ? "Admin" : "Member"}
            </div>
          </div>
        </div>
        <div className="p-1.5">
          <Link href="/user/profile" className="w-full flex items-center gap-2.5 py-2 px-3 rounded-lg border-none bg-transparent cursor-pointer font-jakarta text-[0.8375rem] font-medium text-foreground no-underline transition-colors duration-150 text-left hover:bg-green-500/10 group" onClick={onClose}>
            <Settings className="w-4 h-4 text-muted-foreground shrink-0 transition-colors duration-150 group-hover:text-green-500" />
            Pengaturan Profil
          </Link>
          <Link href="/" className="w-full flex items-center gap-2.5 py-2 px-3 rounded-lg border-none bg-transparent cursor-pointer font-jakarta text-[0.8375rem] font-medium text-foreground no-underline transition-colors duration-150 text-left hover:bg-green-500/10 group" onClick={onClose}>
...
export function UserNavbar({ user }: UserNavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-[39] h-16 flex items-center justify-between px-5 md:px-6 bg-background/92 border-b border-border backdrop-blur-md shadow-[0_1px_0_rgba(34,197,94,0.05),0_2px_8px_rgba(0,0,0,0.04)] font-jakarta gap-4 max-sm:px-4">
      {/* Left: breadcrumb */}
      <nav className="flex items-center gap-0 flex-1 min-w-0 overflow-hidden" aria-label="Breadcrumb">
        {crumbs.map((crumb, i) => (
          <span key={crumb.href} className="flex items-center gap-0 min-w-0 shrink-0 last:shrink last:min-w-0 max-sm:[&:not(:first-child):not(:last-child):not(:nth-last-child(2))]:hidden">
            {i > 0 && <ChevronRight className="w-[0.85rem] h-[0.85rem] text-muted-foreground opacity-40 shrink-0 mx-0.5" />}
            {crumb.isLast ? (
              <span className="text-sm font-bold text-foreground px-1.5 py-1 whitespace-nowrap overflow-hidden text-ellipsis tracking-[-0.01em]">{crumb.label}</span>
            ) : (
              <Link href={crumb.href} className="text-[0.8rem] font-medium text-muted-foreground no-underline px-1.5 py-1 rounded-md transition-colors duration-150 whitespace-nowrap hover:text-foreground hover:bg-green-500/10">
                {crumb.label}
              </Link>
            )}
          </span>
      ...
      <div className="flex items-center gap-2 shrink-0 ml-auto">
        <div className="relative">
          ...
          >
            <Bell />
            {unreadCount > 0 && <span className="absolute top-[5px] right-[5px] w-2 h-2 rounded-full bg-green-500 border-2 border-background animate-pulse shadow-[0_0_0_0_rgba(34,197,94,0.4)]" />}
          </button>
          {notifOpen && (
            <NotifDropdown
        ...
        <div className="relative">
          ...
          >
            <div className="w-full h-full rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center text-sm font-bold text-white tracking-[-0.01em]">{getInitials(user?.name)}</div>
          </button>
          {profileOpen && (
            <ProfileDropdown
              user={user}
